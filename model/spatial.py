...
class SpatialEncoder(nn.Module):
    ...
    Default args for human
        ...
        "n_kpt": 24
    '''
    def __init__(self, sp_level=3, sp_type="rel_z_decay", scale=1.0, n_kpt=24, sigma=0.1):
    ...
    def forward(self, cxyz, kptxyz):
        '''
        get spatial encoding in camera space
        :param cxyz [BV,N,3], xyz in camera space
        :param kptxyz [BV,K,3], keypoints loc in camera space
        :return 
            embeding [BV,K*(2*n_levels+1),N], where n_levels=3
        '''
        B, N = cxyz.shape[:2]
        K = kptxyz.shape[1]

        dz = cxyz[:, :, None, 2:3] - kptxyz[:, None, :, 2:3] #[BV, N, K, 1]
        dxyz = cxyz[:, :, None] - kptxyz[:, None, :] #[BV, N, K, 3]

        # (B, N, K)
        weight = torch.exp(-(dxyz**2).sum(-1) / (2.0 * (self.sigma**2))) #[BV, N, K, 3]
