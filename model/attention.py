'''
copy from https://github.com/DSaurus/DeepMultiCap/blob/main/lib/model/attention.py
'''
...
class Encoder(nn.Module):
    ...
    def forward(self, feats_embedding, return_attns=False):
        ...
        :param return_attns:
        :return: enc_output: (bs, num_views, dim)
        """
        enc_slf_attn_list = []
...
if __name__ == '__main__':
    ...
    encoder = Encoder(2, 6, 32, 32, 256, 256)
    input = torch.rand(5, 4, 256)
    output, *attn = encoder.forward(input, True)
    print(output.shape)
    print(len(attn))
    print(len(attn[0]))
    print(len(attn[0][0]))
    print(len(attn[0][0][0]))
