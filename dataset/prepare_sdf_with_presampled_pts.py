...
flags.DEFINE_string('input_pts_dir', None, 'The path to the input mesh')
...
def process_one_mesh(input_dir, input_pts_dir, input_name, output_dir):
    input_mesh = os.path.join(input_dir, input_name, '%s.obj'%input_name)
    print(f"Processing {input_mesh}")
    mesh = trimesh.load(input_mesh)

    input_space_pts = np.load(os.path.join(input_pts_dir, input_name, 'space.npy'))[:, :3]
    input_surface_pts = np.load(os.path.join(input_pts_dir, input_name, 'surface.npy'))[:, :3]

    
    space_sdfs = mesh_to_sdf(mesh, input_space_pts, 
    ...
    # surface_sdfs = mesh_to_sdf(mesh, input_surface_pts, 
    #     surface_point_method='scan',
    #     scan_count=100,
    #     scan_resolution=400,
    #     sign_method='depth',
    ...
    uniform_points = input_space_pts
    uniform_sdfs = space_sdfs
    # surface_points = input_surface_pts
    # surface_sdfs = surface_sdfs
    ...
    np.savez(output_npz, uniform_points=uniform_points, uniform_sdfs=uniform_sdfs)
...
def main(argv):
    ...
    input_names = sorted(os.listdir(FLAGS.input_dir))[516:517]
