...
import cv2
from utils.common_utils import crop_padding, set_random_seed
...
def make_dataset(opt, phase):
    ...
    path_sdf = '/home/lujiawei/workspace/dataset/thuman2_rescaled_unit_sphere_sdf'
    ...
    if opt.dataset=='Thuman2':
        ...
        return -1
    dataset = Thuman2_PIFU_Dataset_sdf(
        opt = opt,
        phase = phase,
        path_root = path_root,
        path_albedo = osp.join(path_root,  'ALBEDO') ,
        path_mask = osp.join(path_root, 'MASK') ,
        path_lit = osp.join(path_root,  'RENDER') ,
        path_transport = osp.join(path_root,  'H') ,
        ...
        path_normal = osp.join(path_root, 'NORMAL') ,
        path_param = osp.join(path_root, 'PARAM') ,
...
class Thuman2_PIFU_Dataset_sdf(data.Dataset):
    def __init__(
        self, opt, phase, path_root, path_albedo,
        path_mask, path_lit, path_transport, path_obj,path_pts, path_sdf, path_smpl, path_smpl_joints,
        path_normal, path_param, path_shading,
    ...
    def __getitem__(self, index):
        ...
        import time
        t0 = time.time()
        render_item = self.get_render(subject_id, self.opt.num_views, yid, pid, color_aug=color_aug, angle_step=self.opt.angle_step, random_sample=self.opt.random_multiview, random_aug_offset=self.opt.random_aug_offset)
        ...
        if not self.opt.offline_sample:
            ...
            if self.num_sample_inout:
                ...
                sample_pt = {
                    'samples': torch.Tensor(this_space_pt[random_index, :3].T).float(), # N3
            ...
            if self.num_sample_surface:
                ...
                surface_pt = {
                    'surface_samples': torch.Tensor(this_surface_pt[random_index, :3].T).float(), # N3
                    'surface_normals': torch.Tensor(this_surface_pt[random_index, 3:].T).float() # N3 [-1, 1]
                }
                if self.use_smpl:
    ...
    def get_sample_colors(self, subject_id, n_sample_color, view_ids, pid=0):
        ...
        #     uv_render_path = os.path.join(self.path_uv_lit, subject, '%d_%d_%02d.jpg' % (yaw, pitch, 0))
        #     uv_shading_path = os.path.join(self.path_uv_shading, subject, '%d_%d_%02d.jpg' % (yaw, pitch, 0))
        #     # [H, W, 3] 0 ~ 1 float rgb
        ...
        #     uv_shading = cv2.imread(uv_shading_path)
        #     uv_shading = cv2.cvtColor(uv_shading, cv2.COLOR_BGR2RGB) / 255.0
        #     uv_shading = uv_shading.reshape((-1, 3))
        ...
        #     surface_shadings = uv_shading[uv_mask]
        #     ### In these few lines we flattern the masks, positions, and normals
        #     surface_shadings = surface_shadings[sample_list].T

        #     # Samples are around the true surface with an offset
            
        #     rgbs_render_color = torch.Tensor(surface_colors).float()
        #     rgbs_shading_color = torch.Tensor(surface_shadings).float()
        #     uv_render_list.append(rgbs_render_color)
        #     uv_shading_list.append(rgbs_shading_color)
        # render_colors = torch.stack(uv_render_list, dim=0)
        # render_shadings = torch.stack(uv_shading_list, dim=0)
    ...
    def get_render(self, subject_id, num_views=1, yid=0, pid=0, color_aug=None, angle_step=120, random_sample=False,random_aug_offset=0,shuffle=True):
        '''
        ...
        for offset in range(num_views):
            aug_offset = random.choice(list(range(-random_aug_offset, random_aug_offset+1))) # [-12, -6, 0, 6, 12]
            view_ids += [
                self.yaw_list[(yid + angle_step * offset // self.yaw_angle_step + aug_offset) % len(self.yaw_list)]
                ]
        # view_ids = [0, 18, 36]
        # view_ids = [36,54,72]
        # view_ids = [6, 24, 42]
        # view_ids = [24,42,60]
        # view_ids = [0, 30, 60, 90, 120, 150]
        # view_ids = [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]
        # view_ids = [24]
        # view_ids = [228, 246, 264]
        # view_ids = [222, 234, 246]
        # view_ids = [222, 282, 342]
        if random_sample:
            view_ids = list(np.random.choice(self.yaw_list, num_views, replace=False))
        ...
        cam_direction_list = []
        for vid in view_ids:
            random_aug = random.choice(list(range(5))) if self.opt.use_augmentation else 0
            param_path = os.path.join(self.path_param, subject, '%d_%d_%02d.npy' % (vid, pitch, random_aug))
            # render_path = os.path.join(self.path_lit, subject, '%d_%d_%02d.png' % (vid, pitch, random_aug))
            # albedo_path = os.path.join(self.path_albedo, subject, '%d_%d_%02d.png' % (vid, pitch, random_aug))
            render_path = os.path.join(self.path_lit, subject, '%d_%d_%02d.jpg' % (vid, pitch, random_aug))
            albedo_path = os.path.join(self.path_albedo, subject, '%d_%d_%02d.jpg' % (vid, pitch, random_aug))
            normal_path = os.path.join(self.path_normal, subject, '%d_%d_%02d.png' % (vid, pitch, random_aug))
            shading_path = os.path.join(self.path_shading, subject, '%d_%d_%02d.png' % (vid, pitch, random_aug))
            mask_path = os.path.join(self.path_mask, subject, '%d_%d_%02d.png' % (vid, pitch, random_aug))
            # loading calibration data
            param = np.load(param_path, allow_pickle=True)

            # pixel unit / world unit
            ortho_ratio = param.item().get('ortho_ratio')
            if self.opt.use_perspective:
            ...
            R = param.item().get('R')
            # sh coefficients
            sh = param.item().get('sh') # (9,3)

            

            cam = Camera(width=self.load_size, height=self.load_size, fov=self.opt.fov)
            cam.ortho_ratio = ortho_ratio
            # cam.near = -1000
            # cam.far = 1000
            model_radius_bound = 1
            distance_cam = model_radius_bound / np.sin(np.radians(self.opt.fov/2))
            # distance_cam = 2
            cam.near = self.opt.near
            cam.far = self.opt.far
            cam.sanity_check()
            cam.right = R @ np.array([1, 0, 0])
            cam.up = R @ np.array([0, 1, 0])
            cam.direction = R @ np.array([0, 0, -1])
            cam.center = R @np.array([0, 0, distance_cam])
            ...
            rot_mat = cam.get_rotation_matrix()
            
            translate_vec = -np.dot(rot_mat, cam.center)
            cam_extrinsic = np.eye(4)
            cam_extrinsic[:3, :3] = rot_mat
            cam_extrinsic[:3, 3] = translate_vec


            # projection_mat, view_mat = cam.get_gl_matrix()
            projection_mat, view_mat = intrinsic, extrinsic
            a = projection_mat @ np.array([0,0,1,1])

            gl_2_cv_matrix = np.eye(4)
            gl_2_cv_matrix[1,1] = -1

            normalize_matrix = np.eye(4)
            normalize_matrix[:3, :3] = scale*np.eye(3)
            normalize_matrix[:3, 3] = -scale*center

            extrinsic =  view_mat @ normalize_matrix
            ...
            if self.opt.use_perspective:
                intrinsic = gl_2_cv_matrix @ projection_mat
            else:
                ## for orthographic camera we can do augmentations
                # extrinsic = model_mat
                ...
                scale_intrinsic[0, 0] = 1 / ortho_ratio
                scale_intrinsic[1, 1] = -1 / ortho_ratio
                # scale_intrinsic[2, 2] = scale / ortho_ratio
                scale_intrinsic[2, 2] = 1 / ortho_ratio # here the negative is omit because of the ortho projection and axis_adj
                ...
                if self.is_train:
                    # do augmentation
                    pad_size = int(self.padding_rate * self.load_size)
                    render = ImageOps.expand(render, pad_size, fill=0)
                    mask = ImageOps.expand(mask, pad_size, fill=0)
                    albedo = ImageOps.expand(albedo, pad_size, fill=0)
                    shading = ImageOps.expand(shading, pad_size, fill=0)
                    w, h = render.size
                    tw, th = self.load_size, self.load_size
                    assert pad_size == (w-tw)//2
                    assert pad_size == (h-th)//2
                    if self.random_flip and np.random.rand() > 0.5:
                        scale_intrinsic[0,0] *= -1
                        render = transforms.RandomHorizontalFlip(p=1.0)(render)
                        mask = transforms.RandomHorizontalFlip(p=1.0)(mask)
                        albedo = transforms.RandomHorizontalFlip(p=1.0)(albedo)
                        shading = transforms.RandomHorizontalFlip(p=1.0)(shading)
                    ...
                    if self.random_scale:
                        ...
                        albedo = albedo.resize((w, h), Image.BILINEAR)
                        shading = shading.resize((w, h), Image.BILINEAR)
                    ...
                    # assume we have dx = 0.1 pad = 5 pixel, means the cropped image moves left (-dx)
                    ...
                    # crop [0.8pad ~ 1.2pad, 512 + 0.8pad ~ 1.2pad]
                    render = render.crop((x1, y1, x1 + tw, y1 + th))
                    albedo = albedo.crop((x1, y1, x1 + tw, y1 + th))
                    shading = shading.crop((x1, y1, x1 + tw, y1 + th))
            ...
            # render = render.resize((self.opt.load_size, self.opt.load_size), Image.BILINEAR)
            # albedo = albedo.resize((self.opt.load_size, self.opt.load_size), Image.BILINEAR)
            # shading = shading.resize((self.opt.load_size, self.opt.load_size), Image.BILINEAR)
            # mask = mask.resize((self.opt.load_size, self.opt.load_size), Image.NEAREST)
            ...
            cam_extrinsic = torch.Tensor(view_mat).float()
            normalize_matrix = torch.Tensor(normalize_matrix).float()
            cam_center = torch.Tensor(cam.center).float()
            cam_direction = torch.Tensor(cam.direction).float()
            sh = torch.Tensor(sh).float()
            ...
            shading = self.rgb2tensor(shading) # [3, H, W]
            # green = torch.zeros_like(render)
            # green[1] = 1.0
            # render = mask.expand_as(render) * render + (1-mask).expand_as(render) * green
            render = mask.expand_as(render) * render
            albedo = mask.expand_as(albedo) * albedo
            normal = mask.expand_as(normal) * normal
            shading = mask.expand_as(shading) * shading
            ...
            normal_matrix_list.append(normalize_matrix)
            mask_list.append(mask)
            cam_extrinsic_list.append(cam_extrinsic)
            cam_center_list.append(cam_center)
            cam_direction_list.append(cam_direction)

        global_lights = torch.stack(sh_list, dim=0)
        imgs = torch.stack(render_list, dim=0)
        albedos = torch.stack(albedo_list, dim=0)
        normals = torch.stack(normal_list, dim=0)
        shadings = torch.stack(shading_list, dim=0)
        ...
        normal_matrices = torch.stack(normal_matrix_list, dim=0)
        masks = torch.stack(mask_list, dim=0)
        cam_extrinsics = torch.stack(cam_extrinsic_list, dim=0)
        cam_centers = torch.stack(cam_center_list, dim=0)
        cam_directions = torch.stack(cam_direction_list, dim=0)
        z_center = torch.zeros(3, 1)
        return {
            'view_ids': view_ids,
            'global_light': global_lights,
            'z_center': z_center,
            'img': imgs,
            # 'img': shadings,
            'albedo': albedos,
