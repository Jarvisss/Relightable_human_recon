...
def export_materials(mesh_fpath, albd_fn, rough_fn, spec_fn, out_dir, max_num_pts=320000, texture_H=2048, texture_W=2048, n_samples = 5*10**6):
    ...
    for i in range(5):
        ...
        points, points_uv = sample_surface(
            vertices, face_vertices, texturecoords, face_texturecoords, n_samples=n_samples
        )
        ...
        for points_split in torch.split(points, max_num_pts, dim=0):
            # import pdb
            # pdb.set_trace()
            points_split = points_split.transpose(0,1)[None,:,:]
            with torch.set_grad_enabled(False):
                diffuse_albedo = albd_fn(points_split)[0,...].transpose(0,1)
                if spec_fn is not None:
                    specular_albedo, specular_roughness = spec_fn(points_split)[0].T,rough_fn(points_split)[0].T
                else:
                    specular_albedo, specular_roughness = diffuse_albedo.clone(),diffuse_albedo.clone()[:,:1]
                # import pdb
                # pdb.set_trace()
