...
def batch_eval(points, calibs, eval_func, num_samples=512 * 512 * 512):
    num_pts = points.shape[1]
    sdf = np.zeros(num_pts)
    confidences = np.zeros(num_pts)

    num_batches = num_pts // num_samples
    for i in tqdm(range(num_batches)):
        sdf[i * num_samples:i * num_samples + num_samples] = eval_func(
            points[:, i * num_samples:i * num_samples + num_samples], calibs)
    if num_pts % num_samples:
        sdf[num_batches * num_samples:] = eval_func(points[:, num_batches * num_samples:], calibs)

    return sdf

def batch_eval_multi(points, eval_func, num_samples=512 * 512 * 512):
    num_pts = points.shape[1]
    ...
    for i in range(num_batches):
        sdf[i * num_samples:i * num_samples + num_samples] = eval_func(points[:, i * num_samples:i * num_samples + num_samples])[0].view(-1).contiguous()
    if num_pts % num_samples > 0:
        sdf[num_batches * num_samples:] = eval_func(points[:, num_batches * num_samples:])[0].view(-1).contiguous()
    return sdf
...
###
def eval_grid_multi(coords, eval_func, num_samples=512 * 512 * 512):
    resolution = coords.shape[1:4]
    coords = coords.reshape([3, -1])
    sdf = batch_eval_multi(coords, eval_func, num_samples=num_samples)
    return sdf.reshape(resolution)
...
def save_samples_color(fname, points, points_colors, max_num=100000):
    '''
    Save the visualization of sampling to a ply file.
    Red points represent positive predictions.
    Green points represent negative predictions.
    :param fname: File name to save
    :param points: [N, 3] array of points
    :param points_colors: [N, 3] array of point color range [0-255]
    :return:
    '''
    import os
    dir = os.path.dirname(fname)
    ...
    if to_save.shape[0] > max_num:
        indices = np.random.choice(to_save.shape[0], size=max_num)
        to_save = to_save[indices,:]
    return np.savetxt(fname,
                      to_save,
                      fmt='%.6f %.6f %.6f %d %d %d',
                      comments='',
